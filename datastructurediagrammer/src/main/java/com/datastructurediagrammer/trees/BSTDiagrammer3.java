 ...
 * based on that image.
 * 
 * in the "image" 2 dimensional ArrayList each list contains wrapped nodes with added members
...
public class BSTDiagrammer3<T extends Comparable<T>> implements DataStructureDiagrammer<BinarySearchTree<T>> {
    /**
     * NodeWrapper as the name suggests is a wrapper class for BSTNode objects that adds some more information
    ...
    class NodeWrapper { 
        ...
        // which the node has been drawn, 0,0 by default but then assigned their location.
        public int x;
        public int y;
     ...
     * to the diagrammer's image data member. Each node is wrapped with diagramming info
     ...
     * diagramming info.
     * @param wrapperArray The 2 dimensional ArrayList of NodeWrapper type in which wrappers are to be stored.
     * @param currentNode The current (wrapped!) node being worked on.
     * @param level The current level of the tree being worked on. (Where root level is 0, increasing going down when visualized)
    ...
    public BufferedImage renderDiagram(BinarySearchTree<T> tree, String title) {
        ...
        // Most nodes on any given level. This should help us determine width of the image.
        int maxLevelNodes = 0;
        ...
        for (int i = 0; i < treeImage.size(); ++i) { 
            ...
            for (int j = 0; j < nodesOnCurrLevel; ++j) { 
                ...
                //TODO: Do not remember what this additional x-nudge does, or if it really helps. We could axe it.
                elemX += (xIncrement / 2);
                ...
                // Add the (x,y) position and width to the wrapper so children nodes can access it.
                currWrapper.x = elemX;
                currWrapper.y = elemY;
                ...
                if (currWrapper.parent != null) {
                    // The centre of the top of the current node: its x-value plus half a node width.
    ...
    /**
     * Helper method which takes a BST, a title, and an absolute path, renders a diagram
