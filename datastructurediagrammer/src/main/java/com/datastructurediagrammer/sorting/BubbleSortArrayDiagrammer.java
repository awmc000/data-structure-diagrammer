...
/**
 * Generates diagrams of each step as it performs a bubble sort on the given
 * array.
 * 
 * @author Alex McColm
 */
public class BubbleSortArrayDiagrammer<T extends Comparable<T>> implements SortDiagrammer<T> {
    ...
    // If true, the diagrammer will write the value of i & j, and if a swap is
    // happening on the diagram.
     ...
     * @param dirpath Absolute path of the system directory in which to create a
     *                folder of images of stages of bubblesort
     */
    @Override
    public ArrayList<String> renderSortingOperation(T[] array, String title, String dirpath) {
        ...
        // The Bubble Sort operation itself happens here.
        for (int i = 0; i < array.length - 1; ++i) {
            for (int j = 0; j < array.length - i - 1; ++j) {
                ...
                if (array[j].compareTo(array[j + 1]) > 0) {
                    ...
                    // If writing steps is enabled, and there was a swap made, let's write that to
                    // the top left.
                    if (writesSteps) {
